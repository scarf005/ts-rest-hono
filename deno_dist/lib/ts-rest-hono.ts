...
import { StatusCode } from "https://deno.land/x/hono@v3.10.1/utils/http-status.ts"
...
type AppRouteInput<T extends AppRoute> =
  & ServerInferRequest<
    T,
    IncomingHttpHeaders
  >
...
export type AppRouteImplementation<
  T extends AppRoute,
  Env extends HonoEnv,
> = T extends AppRouteMutation ? AppRouteMutationImplementation<T, Env>
  : T extends AppRouteQuery ? AppRouteQueryImplementation<T, Env>
  : never

export type RecursiveRouterObj<T extends AppRouter, Env extends HonoEnv> = {
  [TKey in keyof T]: T[TKey] extends AppRouter
    ? RecursiveRouterObj<T[TKey], Env>
    : T[TKey] extends AppRoute ? AppRouteImplementation<T[TKey], Env>
    : never
}
...
export type ResolvableOption = Options<HonoEnv>[
  keyof Pick<
    Options,
